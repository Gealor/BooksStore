...
def test_authorized_and_me():
    form_data = {"username": "ivan.petrov@example.com", "password": "Petrov2025!"}
...
def test_authorized_and_lending_book():
    form_data = {"username": "maria.smirnova@example.org", "password": "Sm1rnova#88"}
    ...
    response_2 = client.post(
        "/api/business/lending-book", params=params, headers=headers
    )
    assert response_2.status_code == 200
...
def test_authorized_and_return_book():
    ...
    response_2 = client.patch(
        "/api/business/return-book", params=params, headers=headers
    )
    assert response_2.status_code == 200
