...
from core.schemas.users import UserDelete, UserRead, UserUpdate
...
class UserService:
    ...
    def get_my_active_books(
        ...
        result = BorrowedBookRepository(session=session).get_active_borrowed_books_by_user_id(user_id)
        if not result:
    ...
    def get_history_books(
        ...
        result = BorrowedBookRepository(session=session).get_history_about_books_by_user_id(user_id)
